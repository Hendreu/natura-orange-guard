...
import { fmt, severityOrder, severityToken, squadsQueryOptions, teamNames } from "@/lib/sla-data";
...
function Squads() {
  const { data: teams = {}, isLoading, isError } = useQuery(squadsQueryOptions());
  const totalVulns = teamNames.reduce((a, t) => a + (teams[t]?.kpis.vulns ?? 0), 0);
  const totalAssets = teamNames.reduce((a, t) => a + (teams[t]?.kpis.assets ?? 0), 0);
  const avgQds = Math.round(
    teamNames.reduce((a, t) => a + (teams[t]?.kpis.qds ?? 0), 0) / teamNames.length,
  );
    ...
    >
      ...
      {isError ? (
        <div className="slab corner-cut p-6 text-center">
          <h2 className="stencil text-sm text-critica">Erro ao carregar squads</h2>
          <p className="mt-2 text-xs text-muted-foreground">Falha na consulta à base Qualys.</p>
        </div>
      ) : isLoading || Object.keys(teams).length === 0 ? (
        <div className="grid gap-4 lg:grid-cols-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <article key={i} className="slab corner-cut p-5 animate-pulse">
              <div className="h-6 w-1/3 bg-steel" />
              <div className="mt-3 h-3 w-1/2 bg-steel" />
              <div className="mt-6 h-4 w-full bg-steel" />
              <div className="mt-4 grid grid-cols-3 gap-3 border-t border-border pt-4">
                <div className="h-8 bg-steel" />
                <div className="h-8 bg-steel" />
                <div className="h-8 bg-steel" />
              </div>
            </article>
